...
"""
Train.py
This script trains a model in the ISIC 2019 project.

Author      K.Loaiza
Comments    Created: Thursday, May 12, 2020
"""
...
parser.add_argument('--fold_start', default=1, help="change only if resume training from fold different than 1", type=int)


def train_eval(fold, dataloaders, dataset_sizes, net, criterion, optimizer, scheduler, num_epochs):
    """
...
if __name__ == '__main__':
    ...
    for fold in range(args.fold_start, args.folds+1):
        ...
        fname = os.path.join(args.data_dir, f'train{fold}.csv')
        train = pd.read_csv(fname)
        print(fname)
        fname = os.path.join(args.data_dir, f'val{fold}.csv')
        ...
        train_eval(fold, dataloaders, dataset_sizes, net, criterion, optimizer, scheduler, num_epochs=params.num_epochs)
